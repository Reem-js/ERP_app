...
class testWalletController extends Controller {
    ...
    {
        $balance = UserWallet::where('user_id',10)->first()->total_value;
        $transactions = UserWalletTransaction::where('user_wallet_id', 10)->get();
    ...
    {
        $userWallet = UserWallet::where('user_id',10)->first();
        $transactionData = [
    ...
    {
        $userWallet = UserWallet::where('user_id',10)->first();
        $clientWallet = ClientWallet::where('client_id',10)->first();
        $transactionData = [
            'model_type' => 'Models\Sale',
            'model_id' => 6,
            'date'=>$request->date,
